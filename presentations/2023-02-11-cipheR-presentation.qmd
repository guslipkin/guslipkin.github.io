...
## `caesar.R` {.smaller auto-animate="true"}
...
Handle any errors, make sure `x` is a character vector, and split `x` into individual characters.

``` {.r code-line-numbers="3-22"}
caesar <- function(x, n = 1, preserve_spaces = TRUE, dict = NULL, preset = NULL) {
...
Create the dictionary for shifting as needed.

``` {.r code-line-numbers="4-23"}
caesar <- function(x, n = 1, preserve_spaces = TRUE, dict = NULL, preset = NULL) {
  ...
...
## `atbash.R` {.smaller auto-animate="true"}
...
## `railfence.R` {auto-animate="true"}

Set up the user facing and internal functions.

```{.r code-line-numbers="1,3,5-6,8,10-11,13"}
...
## `railfence.R` {.smaller auto-animate="true"}

Handle errors and direct to the appropriate internal function.
...
## `vigenere.R` {.smaller auto-animate="true"}

Handle errors

```{.r code-line-numbers="1-20,23"}
...
Prep for helper functions and dispatch.
...
The following two functions, `.get_letter` and `.rep_key` are at the bottom of the source file, but are used in the encrypt and decrypt methods so it makes more sense to show them now.

## `vigenere.R`

A helper function to return the position of a letter.
...
A helper function to recycle the key to match input length

```{.r code-line-numbers="2-20"}
...
#' @keywords internal
...
A helper function for encryption

```{.r code-line-numbers="2-25"}
...
#' @keywords internal
...
A helper function for decryption

```{.r code-line-numbers="2-24"}
...
#' @keywords internal
...
Handle errors then format the string and key to be passed to `vigenere()`
